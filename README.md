...
<p><em><strong>Random Number Generator for Monte Carlo Algorithm</strong></em></p>
<p>To generate the random numbers for the Monte Carlo algorithm, we first generated a vector of random numbers using the boson sampling simulation. We then generated a second vector of random numbers using a NumPy function. To produce the vector of random numbers used for the Monte Carlo algorithm, we multiplied each component in the NumPy vector with the boson sampling vector to produce a "combo" vector.
...
<p>In the example above, we randomly sample 1000 points and count about 339 landed underneath the curve. Our random numbers go from x = -1 to x = 1 and y = 0 to y = 1, meaning the integral of this function over the given interval is approximately 0.678, only 1.7% off from the actual value of 2/3.</p>
